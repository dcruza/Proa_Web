...
namespace Proa_Web.UserControl
{
    ...
    public partial class casoPaciente : System.Web.UI.UserControl
    {
        /// <summary>
        /// Id del paciente
        /// </summary>
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidarCampos())
                {
                    ...
                    msgBox.ShowMsg(Page.Title, "Los datos de la estancia se almacenaron correctamente", clsGlobals.messageTypes.info);

                    //ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "Close", "Close();", true);
                }
                else
                {
        ...
        private bool ValidarCampos()
        {
            ...
            else if (!int.TryParse(txtCaso.Text, out nc))
            {
                valido = false;
                sb.AppendLine(" * N° Caso");
            }
            else if (ddlServicio.SelectedItem == null)
            {
                valido = false;
                sb.AppendLine(" * Servicio");
            }
            else if (ddlEspec.SelectedItem.Text == string.Empty)
            {
                valido = false;
                sb.AppendLine(" * Especialidad");
            }
            else if (grdDx.SelectedIndex < 0)
            {
                valido = false;
